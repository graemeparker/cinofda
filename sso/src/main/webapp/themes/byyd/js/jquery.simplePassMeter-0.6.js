...
;(function($) {
  ...
  function setup(n, o) {
    ...
    var b = $("<div id='"+mId+"' class='simplePassMeter' aria-controlled>")
      .append("<p><span class=''></span><span class='simplePassMeterText'></span></p>"+
    ...
    for (var k in rq) {
      if (rq[k] && rq[k].value) {
        if (rq[k].regex && rq[k].regex.length > 0) {
          ...
          if (!re.test(p)) {
        	// disabled appending due to conflicts with registration
            //if (m.length > 0) { m += '<br />'; }
  ...
  $.fn.simplePassMeter.defaults = {
    ...
    'requirements': {                   // OBJECT  All requirements that the password must fulfill.
      ...
      'minLength': {
        ...
        'message': 'Keep typing' // STRING  The message to display to the user if they do not meet the requirement. It can contain the special sequence %V which will be replaced by the value parameter for this requirement.
      },
      'noMatchField': {
    ...
    'ratings': [                              // ARRAY   The categories for the ratings. When an input hits the "minScore" for a given rating, its values will be used. NOTE THAT THESE MUST BE IN ORDER BY THE "minScore" ATTRIBUTE.
      {'minScore': 0,                         // NUMBER  The minimum score to hit to trigger this rating.
       ...
       'text': 'Use numbers and letters'	  // STRING  The text to place in the meter UI when this rating is triggered.
      },
      {'minScore': 25,
       'className': 'meterWarn',
       'text': 'Your password is OK, could be better!'
      },
      {'minScore': 50,
       'className': 'meterGood',

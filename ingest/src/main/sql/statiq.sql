...
CREATE TABLE `device` (
  `id` int(11) NOT NULL,
  `raw_did` char(36) CHARACTER SET latin1 COLLATE latin1_general_cs NOT NULL,
  `country_id` int(11) NOT NULL DEFAULT '0',
  `device_type_id` int(11) DEFAULT '6',
  PRIMARY KEY (`id`),
  UNIQUE KEY `raw_did` (`raw_did`, `device_type_id`)
...
select
  ...
  in_output_id
from segment_device sd
  inner join device on device.id = sd.device_id

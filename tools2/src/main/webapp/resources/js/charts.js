...
// !Modified - now it won't throw an error when there is no data
...
//                    };
    ...
    jsDate.prototype.getUtcOffset = function() {
        return this.proxy.getTimezoneOffset() * 6000000;
    };
...
(function($) {
    ...
    $.jqplot.Highlighter.postPlotDraw = function() {
        ...
        elem = null;
        this.plugins.highlighter._tooltipElem.addClass('jqplot-highlighter-tooltip ui-shadow');
    ...
    // !Modified - added custom config options for highlited marker
    function draw(plot, neighbor) {
        ...
        if (hl.markerRenderer.customOptions) {
            for (prop in hl.markerRenderer.customOptions) {
                if (hl.markerRenderer.customOptions.hasOwnProperty(prop)) {
                    smr[prop] = hl.markerRenderer.customOptions[prop];
                }
            }
            if (smr.bgColor) {
                var cmr = hl.markerRenderer;
                cmr.color = smr.bgColor;
                cmr.style = 'filledCircle';
                cmr.init();
                cmr.draw(s.gridData[neighbor.pointIndex][0], s.gridData[neighbor.pointIndex][1], hl.highlightCanvas._ctx);
            }
        }
        ...
        var newrgb = [rgba[0], rgba[1], rgba[2]];
        var defaultAlpha = (rgba[3] >= 0.6) ? rgba[3]*0.6 : rgba[3]*(2-rgba[3]);
        var alpha = smr.alpha || defaultAlpha;
    ...
    function showTooltip(plot, series, neighbor) {
        ...
        if (opts.useAxesFormatters) {
            ...
            var str,
                // !Modified added 2 new variables for tooltip markup
                label = opts.tooltipLabel || ' ',
                unit = opts.tooltipUnit || ' ';


            // !Modified - added date formating to highlighter and 2 variables for label and unit
            if (typeof opts.formatString ===  'string') {

                opts.formatString = opts.formatString.replace(/%label/, label);
                opts.formatString = opts.formatString.replace(/%unit/, unit);

                if (opts.formatDate && /^\d{13}$/.test(neighbor.data[opts.formatDate.itemNo - 1].toString())) {
                    neighbor.data[opts.formatDate.itemNo - 1] = $.datepicker.formatDate(opts.formatDate.dateFormat, new Date(neighbor.data[opts.formatDate.itemNo - 1]));
                }
...
PrimeFaces.widget.LineChart = PrimeFaces.widget.CartesianChart.extend({
    ...
    configure: function() {
        ...
        // !Modified added highlighter format string
        this.cfg.highlighter.formatString = '<div class="inner"><p class="date">%s</p>' +
            '<p class="rate">%label</p><p class="value">%s%unit</p></div>';

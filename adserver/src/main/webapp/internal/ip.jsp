...
<%@ page import="com.quova.data._1.NetworkType" %>
...
if (ip != null) {
    ...
    //MAD-455
    ipinfo = quovaClient.getIpinfo(ip);
	network = ipinfo.getNetwork();
}
...
<td><% if (operator != null) {
	out.print(operator.getName() + " (id=" + operator.getId() + ")"); 
}
else if (network != null && network.getCarrier() != null && network.getCarrier() != "" 
		&& "mobile gateway".equals(network.getIpRoutingType())) {
	out.print(network.getCarrier());
}
else {
	out.print("NOT RESOLVED");
}
...
private static void dumpIPInfo(Ipinfo info, JspWriter out) throws java.io.IOException {
    ...
    String xml = null;
    try {
        JAXBContext jaxbContext = JAXBContext.newInstance("com.quova.data._1");
        Marshaller marshaller = jaxbContext.createMarshaller();
        marshaller.setProperty(Marshaller.JAXB_ENCODING, "UTF-8");
        marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
        marshaller.setProperty(Marshaller.JAXB_FRAGMENT, Boolean.TRUE);
        StringWriter swri = new StringWriter();
        marshaller.marshal(info, swri);
        xml = swri.toString();
    } catch (Exception e) {
        e.printStackTrace();
    }
    ...
    if (xml != null) {
        out.println("<p><b>Raw XML Response:</b><br/><pre>" + xml.replaceAll(">", "&gt;").replaceAll("<", "&lt;") + "</pre></p>");
    }
}

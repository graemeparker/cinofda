...
public class MarginShareCPM_NONrtbTest {

    MarginShareCPM_NONrtb testObj = new MarginShareCPM_NONrtb();
    ...
    public void testAdServed3() {
        ...
        Assert.assertEquals(0.0008823529, out.payout, accuracy);
        Assert.assertEquals(0, out.buyer_premium, accuracy);
        Assert.assertEquals(0, out.direct_cost, accuracy);
        Assert.assertEquals(0, out.tech_fee, accuracy);
        Assert.assertEquals(0, out.data_fee, accuracy);
        Assert.assertEquals(0, out.third_pas_fee, accuracy);
        Assert.assertEquals(0.0020588235, out.dsp_margin, accuracy);
        Assert.assertEquals(0.0020588235, out.cust_margin, accuracy);
        Assert.assertEquals(0, out.campaign_discount, accuracy);
  ...
  //------------------------------------------------------------
    ...
    public void csvAdServed() throws IOException{
        new TestHelper().testCSV(testObj, "MarginShare CPM on NonRTB-AD_SERVED.csv", AdAction.AD_SERVED);
    }
    
    @Test
    public void csvClick() throws IOException{
        new TestHelper().testCSV(testObj,"MarginShare CPM on NonRTB-CLICK.csv", AdAction.CLICK);
    }
